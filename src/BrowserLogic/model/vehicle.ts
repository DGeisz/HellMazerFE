export class Vehicle {
    ...
    update(d_t: number, v_l: number, v_r: number, wallsInFront: [boolean, boolean]) {
        ...
        const new_x = this.x + (d_l * Math.cos(this.angle));
        const new_y = this.y + (d_l * Math.sin(this.angle));
        ...
        if (!(wallsInFront[0] && x_floor !== new_x_floor)) {
            this.x = new_x;
        }

        if (!(wallsInFront[1] && y_floor !== new_y_floor)) {
            this.y = new_y;
        }
